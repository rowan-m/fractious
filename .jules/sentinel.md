...
**Vulnerability:** External links (`<a>` tags) in `index.html` were missing `target="_blank"` and `rel="noopener noreferrer"`.
...
**Prevention:** Always use `target="_blank" rel="noopener noreferrer"` for external links to ensure they open in a new, unlinked context.
