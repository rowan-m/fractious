...
| [`src/InteractionManager.js`](src/InteractionManager.js) | Unified pointer/touch/wheel gesture mathematics (zoom-around-cursor, 2-finger pinch/rotate) and DOM input event bindings.                                                                                                                                    |
...
| [`src/worker.js`](src/worker.js)                         | Web Worker entry point. Computes required bit precision from `scale`, invokes `find_best_anchor` and `calculate_reference`, and transfers `orbit.buffer` back zero-copy.                                                                                     |
| [`wasm/src/lib.rs`](wasm/src/lib.rs)                     | Rust/WASM core using `dashu`. Implements arbitrary-precision coordinate arithmetic (`add_coord`, `sub_coord`), coarse-to-fine perturbation-accelerated anchor search (`find_best_anchor`), and quad-float reference orbit splitting (`calculate_reference`). |
