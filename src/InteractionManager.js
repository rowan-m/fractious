...
export class InteractionManager {
    ...
    bindInputEvents() {
        ...
        inputs.hue.addEventListener('change', () => {
            ...
            if (!isNaN(v)) { this.config.hue = v; this.callbacks.onInteract(false); } else this.updateUI();
        });

        inputs.hueStep.addEventListener('change', () => {
            const v = parseFloat(inputs.hueStep.value);
            if (!isNaN(v)) { this.config.hueStep = v; this.callbacks.onInteract(false); } else this.updateUI();
        });
    }
    ...
    bindButtonEvents() {
        ...
        const buttonActions = {
            ...
            'btn-zoom-in': () => { this.state.targetZoom /= 1.5; this.callbacks.onInteract(true); },
            'btn-zoom-out': () => { this.state.targetZoom *= 1.5; this.callbacks.onInteract(true); },
            'btn-rotate-cw': () => { this.config.rotation += Math.PI / 12; this.callbacks.onInteract(false); },
            'btn-rotate-ccw': () => { this.config.rotation -= Math.PI / 12; this.callbacks.onInteract(false); },
            'btn-cycle-in': () => { this.config.hueStep += 0.05; this.callbacks.onInteract(false); },
            'btn-cycle-out': () => { this.config.hueStep -= 0.05; this.callbacks.onInteract(false); },
            'btn-hue-left': () => { this.config.hue -= 0.05; this.callbacks.onInteract(false); },
            'btn-hue-right': () => { this.config.hue += 0.05; this.callbacks.onInteract(false); },
            'btn-screenshot': () => {
